...
If no DWIM context is found, nil is returned."
  (cond
   ((and-let* ((commits (magit-region-values '(commit branch) t)))
      (deactivate-mark) ; "secondary" side-effect
...
(defun magit-hunk-goto-successor (section arg)
  ;; It's all about the side-effect,
  ;; but the return value is also really important.
  ...
  (cl-flet ((cache (value)
    ...
    (cond
     ...
     ((and-let* ((buffer (find-buffer-visiting file)))
        ;; Treat caching as a side-effect?  Value seems more important here.
